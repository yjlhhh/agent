...
class Dealer:
    ...
    def get_vector(self, txt, emb_mdl, topk=10, similarity=0.1):
        ...
        if qv is None:
            raise Exception("embedding unavailable")
    ...
    def search(self, req, idx_names: str | list[str],
        ...
        if not qst:
            ...
            # 无 embedding 服务时回退到纯文本检索（例如仅配置 DeepSeek）
            matchDense = None
            try:
                matchDense = self.get_vector(qst, emb_mdl, topk, req.get("similarity", 0.1))
            except Exception as e:
                logging.warning(f"vector search unavailable, fallback to text search: {e}")
            ...
            if matchDense is None:
                ...
                total = self.dataStore.getTotal(res)
                logging.debug("Dealer.search TOTAL(text-only): {}".format(total))
                if total == 0:
                    ...
                    total = self.dataStore.getTotal(res)
                    logging.debug("Dealer.search 2 TOTAL(text-only): {}".format(total))
            else:
